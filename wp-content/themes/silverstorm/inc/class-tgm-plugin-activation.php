 ...
 * @version   2.6.1 for parent theme Silverstorm for publication on WordPress.org
...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            // Load class strings.
            $this->strings = array(
                'page_title'                      => __( 'Install Required Plugins', 'silverstorm' ),
                'menu_title'                      => __( 'Install Plugins', 'silverstorm' ),
                /* translators: %s: plugin name. */
                'installing'                      => __( 'Installing Plugin: %s', 'silverstorm' ),
                /* translators: %s: plugin name. */
                'updating'                        => __( 'Updating Plugin: %s', 'silverstorm' ),
                'oops'                            => __( 'Something went wrong with the plugin API.', 'silverstorm' ),
                ...
                'return'                          => __( 'Return to Required Plugins Installer', 'silverstorm' ),
                'dashboard'                       => __( 'Return to the Dashboard', 'silverstorm' ),
                'plugin_activated'                => __( 'Plugin activated successfully.', 'silverstorm' ),
                ...
                'complete'                        => __( 'All plugins installed and activated successfully. %1$s',
                    'silverstorm' ),
                'dismiss'                         => __( 'Dismiss this notice', 'silverstorm' ),
        ...
        public function filter_plugin_action_links_update( $actions ) {
            $actions['update'] = sprintf(
                ...
                esc_attr__( 'This plugin needs to be updated to be compatible with your theme.', 'silverstorm' ),
                esc_html__( 'Update Required', 'silverstorm' )
            );
                    ...
                    if ( true === $GLOBALS['wp_filesystem']->move( $from_path, $to_path ) ) {
                        ...
                        return new WP_Error( 'rename_failed',
                            esc_html__( 'The remote plugin package does not contain a folder with the desired slug and renaming did not work.',
                                'silverstorm' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.',
                                'silverstorm' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
                    }
                ...
                if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
                    ...
                    foreach ( $message as $type => $plugin_group ) {
                        ...
                        $imploded       = empty( $linked_plugins ) ? $last_plugin : ( implode( ', ',
                                ...
                                'silverstorm' ) . ' ' . $last_plugin );

                        $rendered .= sprintf(
                            $line_template,
                            sprintf(
                                translate_nooped_plural( $this->strings[ $type ], $count, 'silverstorm' ),
                                $imploded,
                                $count
                            )
                        );
        ...
        protected function create_user_action_links_for_notice(
            ...
            if ( current_user_can( 'install_plugins' ) ) {
                if ( $install_count > 0 ) {
                    $action_links['install'] = sprintf(
                        $link_template,
                        translate_nooped_plural( $this->strings['install_link'], $install_count, 'silverstorm' ),
                ...
                if ( $update_count > 0 ) {
                    $action_links['update'] = sprintf(
                        $link_template,
                        translate_nooped_plural( $this->strings['update_link'], $update_count, 'silverstorm' ),
                        esc_url( $this->get_tgmpa_status_url( 'update' ) )
                    );
                }
            }
            ...
            if ( current_user_can( 'activate_plugins' ) && $activate_count > 0 ) {
                $action_links['activate'] = sprintf(
                    $link_template,
                    translate_nooped_plural( $this->strings['activate_link'], $activate_count, 'silverstorm' ),
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function get_plugin_advise_type_text( $required ) {
            if ( true === $required ) {
                return __( 'Required', 'silverstorm' );
            }

            return __( 'Recommended', 'silverstorm' );
        }
        ...
        protected function get_plugin_source_type_text( $type ) {
            ...
            switch ( $type ) {
                case 'repo':
                    $string = __( 'WordPress Repository', 'silverstorm' );
                    break;
                case 'external':
                    $string = __( 'External Source', 'silverstorm' );
                    break;
                case 'bundled':
                    $string = __( 'Pre-Packaged', 'silverstorm' );
                    break;
            }
        ...
        protected function get_plugin_status_text( $slug ) {
            if ( ! $this->tgmpa->is_plugin_installed( $slug ) ) {
                return __( 'Not Installed', 'silverstorm' );
            }

            if ( ! $this->tgmpa->is_plugin_active( $slug ) ) {
                $install_status = __( 'Installed But Not Activated', 'silverstorm' );
            } else {
                $install_status = __( 'Active', 'silverstorm' );
            }

            $update_status = '';

            if ( $this->tgmpa->does_plugin_require_update( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
                $update_status = __( 'Required Update not Available', 'silverstorm' );

            } elseif ( $this->tgmpa->does_plugin_require_update( $slug ) ) {
                $update_status = __( 'Requires Update', 'silverstorm' );

            } elseif ( false !== $this->tgmpa->does_plugin_have_update( $slug ) ) {
                $update_status = __( 'Update recommended', 'silverstorm' );
            }
            ...
            /* translators: 1: install status, 2: update status */
                _x( '%1$s, %2$s', 'Install/Update Status', 'silverstorm' ),
                $install_status,
                $update_status
            );
        }
                ...
                switch ( $type ) {
                    case 'all':
                        ...
                        $text = _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>',
                            $count, 'plugins', 'silverstorm' );
                        break;
                    case 'install':
                        /* translators: 1: number of plugins. */
                        $text = _n( 'To Install <span class="count">(%s)</span>',
                            'To Install <span class="count">(%s)</span>', $count, 'silverstorm' );
                        break;
                    case 'update':
                        /* translators: 1: number of plugins. */
                        $text = _n( 'Update Available <span class="count">(%s)</span>',
                            'Update Available <span class="count">(%s)</span>', $count, 'silverstorm' );
                        break;
                    case 'activate':
                        /* translators: 1: number of plugins. */
                        $text = _n( 'To Activate <span class="count">(%s)</span>',
                            'To Activate <span class="count">(%s)</span>', $count, 'silverstorm' );
                        break;
                    default:
                        $text = '';
                        break;
        ...
        public function column_version( $item ) {
            ...
            if ( $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
                $installed = ! empty( $item['installed_version'] ) ? $item['installed_version'] : _x( 'unknown',
                    'as in: "version nr unknown"', 'silverstorm' );
        ...
        public function no_items() {
            echo esc_html__( 'No plugins to install, update or activate.',
                    'silverstorm' ) . ' <a href="' . esc_url( self_admin_url() ) . '"> ' . esc_html__( 'Return to the Dashboard',
                    'silverstorm' ) . '</a>';
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        }
        ...
        public function get_columns() {
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'silverstorm' ),
                'source' => __( 'Source', 'silverstorm' ),
                'type'   => __( 'Type', 'silverstorm' ),
            );

            if ( 'all' === $this->view_context || 'update' === $this->view_context ) {
                $columns['version'] = __( 'Version', 'silverstorm' );
                $columns['status']  = __( 'Status', 'silverstorm' );
            }
        ...
        protected function get_row_actions( $item ) {
            ...
            if ( ! $this->tgmpa->is_plugin_installed( $item['slug'] ) ) {
                ...
                $actions['install'] = __( 'Install %2$s', 'silverstorm' );
            } else {
                // Display the 'Update' action link if an update is available and WP complies with plugin minimum.
                if ( false !== $this->tgmpa->does_plugin_have_update( $item['slug'] ) && $this->tgmpa->can_plugin_update( $item['slug'] ) ) {
                    /* translators: %2$s: plugin name in screen reader markup */
                    $actions['update'] = __( 'Update %2$s', 'silverstorm' );
                }

                // Display the 'Activate' action link, but only if the plugin meets the minimum version.
                if ( $this->tgmpa->can_plugin_activate( $item['slug'] ) ) {
                    /* translators: %2$s: plugin name in screen reader markup */
                    $actions['activate'] = __( 'Activate %2$s', 'silverstorm' );
                }
            }
        ...
        public function wp_plugin_update_row( $slug, $item ) {
            ...
            esc_html__( 'Upgrade message from the plugin author:', 'silverstorm' ),
        ...
        public function get_bulk_actions() {
            ...
            if ( 'update' !== $this->view_context && 'activate' !== $this->view_context ) {
                if ( current_user_can( 'install_plugins' ) ) {
                    $actions['tgmpa-bulk-install'] = __( 'Install', 'silverstorm' );
                }
            }

            if ( 'install' !== $this->view_context ) {
                if ( current_user_can( 'update_plugins' ) ) {
                    $actions['tgmpa-bulk-update'] = __( 'Update', 'silverstorm' );
                }
                if ( current_user_can( 'activate_plugins' ) ) {
                    $actions['tgmpa-bulk-activate'] = __( 'Activate', 'silverstorm' );
                }
            }
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action() ) {
                ...
                if ( empty( $_POST['plugin'] ) ) {
                    if ( 'install' === $install_type ) {
                        $message = __( 'No plugins were selected to be installed. No action taken.', 'silverstorm' );
                    } else {
                        $message = __( 'No plugins were selected to be updated. No action taken.', 'silverstorm' );
                    }
                ...
                if ( empty( $plugins_to_install ) ) {
                    if ( 'install' === $install_type ) {
                        $message = __( 'No plugins are available to be installed at this time.', 'silverstorm' );
                    } else {
                        $message = __( 'No plugins are available to be updated at this time.', 'silverstorm' );
                    }
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( // WPCS: xss ok.
                        ...
                        esc_html( _n( 'The following plugin was activated successfully:',
                            'The following plugins were activated successfully:', $count, 'silverstorm' ) ),
                        $imploded
                    );
            ...
            if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
                ...
                class TGMPA_Bulk_Installer extends Plugin_Upgrader {
                    ...
                    public function activate_strings() {
                        $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'silverstorm' );
                        $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'silverstorm' );
                    }
                        ...
                        if ( 'update' === $this->options['install_type'] ) {
                            ...
                            if ( $this->tgmpa->is_automatic ) {
                                ...
                                $this->upgrader->strings['skin_update_successful'] = __( '%1$s installed and activated successfully.',
                                        'silverstorm' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details',
                                        'silverstorm' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details',
                                        'silverstorm' ) . '</span>.</a>';
